...
func _on_line_edit_text_submitted(text: String):
	...
	var levelvar_value: Variant = null
	if args.size() > 1:
		levelvar_value = args[1]
	...
	if levelvar_value == null and level.levelvar.get_typeof_var(levelvar_name) == TYPE_BOOL:
		var toggled_value: bool = not level.levelvar.get_var(levelvar_name)
		level.levelvar.set_var(levelvar_name, toggled_value)
		return
